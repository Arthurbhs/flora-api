...
app.get('/flora', (req, res) => {
  ...
  if (categoria) {
    const filteredFlora = flora.filter(p => p.categoria.toLowerCase() === categoria.toLowerCase());
    res.json(filteredFlora);
  } else {
    res.json(flora);
  }
});

// Rota para obter uma planta específica por ID
app.get('/flora/:id', (req, res) => {
  const plant = flora.find(p => p.id === parseInt(req.params.id));
  if (plant) {
    res.json(plant);
  } else {
...
// Rota para atualizar uma planta existente por ID
app.put('/flora/:id', (req, res) => {
  const plant = flora.find(p => p.id === parseInt(req.params.id));
  if (plant) {
    plant.especie = req.body.especie;
    plant.categoria = req.body.categoria;
    plant.descricao = req.body.descricao;
    res.json(plant);
  } else {
    res.status(404).send('Planta não encontrada');
  }
...
// Rota para deletar uma planta por ID
app.delete('/flora/:id', (req, res) => {
  const plantIndex = flora.findIndex(p => p.id === parseInt(req.params.id));
  if (plantIndex !== -1) {
    const deletedPlant = flora.splice(plantIndex, 1);
